...
CC_FILE_ERROR LasFilter::saveToFile(ccHObject *entity, const QString &filename,
  ...
  if (theCloud->hasMetaData("LAS_HEADER")) {
    ...
    QString version =
        QString("1.%1").arg(QString::number(lasHeader.versionMinor()));
    saveDialog.setVersionAndPointFormat(version, lasHeader.pointFormat());
    const CCVector3d savedScale(lasHeader.scaleX(), lasHeader.scaleY(),
                                lasHeader.scaleZ());
    saveDialog.setSavedScale(savedScale);
  ...
  if (theCloud->hasMetaData("LAS_HEADER")) {
    ...
    for (const pdal::LasVLR &vlr : lasHeader.vlrs()) {
      ...
      vlrObject["data"] = QByteArray(vlr.data()).toBase64().data();
  ...
  converter.setInput(progressStage);

  try {
    pdal::LasWriter writer;
    writer.setOptions(writerOptions);
    writer.setInput(converter);
...
struct DetermineLasShiftCallback {
  ...
  void operator()(const CCVector3d &point, ccPointCloud &cloud) {
    ...
    if (lasOffsets.norm2() != 0 && (!parameters.coordinatesShiftEnabled ||
...
CC_FILE_ERROR LasFilter::loadFile(const QString &filename, ccHObject &container,
  ...
  openDialog.setInfo(lasHeader.versionMinor(), lasHeader.pointFormat(),
  ...
  CCVector3d lasOffsets(lasHeader.offsetX(), lasHeader.offsetY(),
                        lasHeader.offsetZ());
